...
namespace CNCSS.Vis
{
    ...
    public sealed class MachineVisualCoordinator
    {
        ...
        private ModelVisual3D? _anchoredAdaptiveProximityVisual;
        /// <summary>
        ...
        /// <summary>Цилиндр зоны адаптивной точности на том же узле, что и инструмент.</summary>
        public void SetMountNodeAdaptiveProximity(ModelVisual3D? visual)
        {
            RemoveMountNodeChild(_anchoredAdaptiveProximityVisual);
            _anchoredAdaptiveProximityVisual = visual;
            AttachMountNodeChild(_anchoredAdaptiveProximityVisual);
        }
        ...
        private void ReattachTableMountedVisuals()
        {
            ...
            ReattachMountNodeAdaptiveProximity();
        }

        private void ReattachMountNodeTool() => AttachMountNodeChild(_anchoredToolVisual);

        private void ReattachMountNodeAdaptiveProximity() => AttachMountNodeChild(_anchoredAdaptiveProximityVisual);

        private void AttachTableMountedChild(ModelVisual3D? visual, bool insertFirst = false)
        {
            if (visual == null)
            {
                return;
            ...
            if (insertFirst)
            {
                _table.Root.Children.Insert(0, visual);
            }
            else
            {
