...
namespace CNCSS.UI.ViewModels
{
    ...
    public sealed class StockConstructorViewModel : BaseViewModel
    {
        ...
        public StockConstructorViewModel(StockConstructorConfig? initial = null)
        {
            ...
            SelectedColor = initial?.Color.ToWpfColor() ?? ColorSwatches[0];

            L2Resolution = VoxelConstants.VoxelResolutionMm;
        ...
        private double _l2Resolution;
        public double L2Resolution
        {
            get => _l2Resolution;
            set
            {
                value = Math.Clamp(value, VoxelConstants.VoxelResolutionMm, 1.0);
                if (Math.Abs(_l2Resolution - value) < 1e-12)
                {
                    return;
                }
                _l2Resolution = value;
                OnPropertyChanged();
                OnPropertyChanged(nameof(L2ResolutionText));
            }
        }

        public string L2ResolutionText => _l2Resolution.ToString("0.00", CultureInfo.InvariantCulture);
